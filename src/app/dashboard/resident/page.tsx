    ...
    async function fetchOrdersAndRefunds() {
      ...
      for (const vendorDoc of vendorsSnapshot.docs) {
        const refundsRef = collection(
          firestore,
          `vendors/${vendorDoc.id}/refund_requests`
        )
                  ...
                  <TableCell className="text-right">
                    {order.status === 'Completed' && !order.refundState && (
                      <Button asChild variant="outline" size="sm">
                        <Link
                          href={`/dashboard/resident/refunds/${order.id}?vendorId=${order.vendorId}`}
                        >
