...
export default function ForumThreadPage({
  ...
  const { data: thread, isLoading: isThreadLoading } =
    useDoc<ForumThread>(threadRef)

  const postsQuery = useMemoFirebase(
    () =>
      firestore
        ? query(
            collection(firestore, 'forumThreads', params.id, 'posts'),
  ...
  const { data: posts, isLoading: arePostsLoading } =
    useCollection<ForumPost>(postsQuery)
    ...
    <div className="container mx-auto max-w-4xl px-4 py-8 md:py-12">
      ...
      <header className="mb-8 space-y-4">
        ...
        <div className="flex items-center gap-2 text-muted-foreground">
          ...
          <span>
            {arePostsLoading ? '...' : posts?.length || 0} posts in this
            discussion
          </span>
        </div>
      </header>
      ...
      <div className="space-y-6">
        ...
        {posts?.map((post, index) => {
          ...
          if (isClient) {
            avatarUrl = isFirstPost
              ? thread.authorAvatarUrl
              : index % 2 === 0
                ? userAvatar2?.imageUrl
                : userAvatar1?.imageUrl
            avatarHint =
              index % 2 === 0 ? userAvatar2?.imageHint : userAvatar1?.imageHint
          }
          return (
            <Card
              key={post.id}
