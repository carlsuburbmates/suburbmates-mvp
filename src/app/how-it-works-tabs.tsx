      ...
      <Tabs defaultValue="residents" className="w-full">
        ...
        <TabsContent value="residents" className="mt-10">
          <div className="relative -mx-4 md:mx-0">
            <div
              ref={resRef}
              className="flex gap-4 overflow-x-auto snap-x snap-mandatory md:grid md:grid-cols-3 md:gap-6 md:overflow-visible px-4"
            >
              {residentSteps.map((step, index) => (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <div
                      className={`flex items-center justify-center h-10 w-10 rounded-md ${mergedThemes.residents.frameBg} ${mergedThemes.residents.frameText} ${mergedThemes.residents.frameRing}`}
                    >
                      {step.icon}
                    </div>
            ...
            <div
              className={`pointer-events-none absolute inset-y-0 left-1 flex items-center md:hidden transition-opacity duration-300 ${resAtStart ? 'opacity-0' : 'opacity-60'}`}
            >
            ...
            <div
              className={`pointer-events-none absolute inset-y-0 right-1 flex items-center md:hidden transition-opacity duration-300 ${resAtEnd ? 'opacity-0' : 'opacity-60'}`}
            >
              <div className="rounded-full bg-background/70 backdrop-blur supports-[backdrop-filter]:bg-background/60 ring-1 ring-border shadow-sm p-1">
                <ChevronRight
                  className="h-4 w-4 text-muted-foreground"
        ...
        <TabsContent value="businesses" className="mt-10">
          <div className="relative -mx-4 md:mx-0">
            ...
            >
              {businessSteps.map((step, index) => (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <div
                      className={`flex items-center justify-center h-10 w-10 rounded-md ${mergedThemes.businesses.frameBg} ${mergedThemes.businesses.frameText} ${mergedThemes.businesses.frameRing}`}
                    >
                      {step.icon}
                    </div>
                    <span className="text-[10px] text-muted-foreground">
            ...
            {/* Subtle chevron scroll indicators (fade out when edges reached) */}
            <div
              className={`pointer-events-none absolute inset-y-0 left-1 flex items-center md:hidden transition-opacity duration-300 ${bizAtStart ? 'opacity-0' : 'opacity-60'}`}
            >
              <div className="rounded-full bg-background/70 backdrop-blur supports-[backdrop-filter]:bg-background/60 ring-1 ring-border shadow-sm p-1">
                <ChevronLeft
                  className="h-4 w-4 text-muted-foreground"
            ...
            <div
              className={`pointer-events-none absolute inset-y-0 right-1 flex items-center md:hidden transition-opacity duration-300 ${bizAtEnd ? 'opacity-0' : 'opacity-60'}`}
            >
              <div className="rounded-full bg-background/70 backdrop-blur supports-[backdrop-filter]:bg-background/60 ring-1 ring-border shadow-sm p-1">
                <ChevronRight
                  className="h-4 w-4 text-muted-foreground"
