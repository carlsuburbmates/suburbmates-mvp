...
export default function VendorProfilePage({
  ...
  const handlePurchase = async (listing: Listing) => {
    if (!user || !auth.currentUser) {
      toast({
        variant: 'destructive',
        title: 'Please log in to purchase an item.',
      })
      return
    }
    if (!vendor || !vendor.stripeAccountId || !vendor.paymentsEnabled) {
      toast({
        ...
        description:
          'This vendor has not connected their payment account or is awaiting approval.',
      })
      return
    }
    ...
    try {
      await runTransaction(firestore, async (transaction) => {
        ...
        const newAverageRating =
          (currentAverageRating * currentReviewCount + values.rating) /
          newReviewCount
    ...
    <div>
      ...
      <div className="container mx-auto px-4 pb-16">
          ...
          <div className="flex-1 p-6">
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3 text-muted-foreground">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>
                  {vendor.averageRating
                    ? `${vendor.averageRating.toFixed(1)} stars`
                    : 'No reviews yet'}
              ...
              {vendor.refundPolicyUrl ? (
                ...
                <div className="flex items-center gap-2">
                  <FileText className="h-4 w-4" />
                  <Link href="/policy" className="hover:text-primary underline">
            ...
            {vendor.fulfilmentTerms && (
              <Alert className="mt-6">
                ...
                <AlertDescription>{vendor.fulfilmentTerms}</AlertDescription>
              </Alert>
            )}
          </div>
        </Card>
        ...
        {vendor.paymentsEnabled && (
          <>
            <h2 className="text-[clamp(1.5rem,3.5vw,2.5rem)] font-bold font-headline leading-tight mb-6">
              Our Offerings
            </h2>
            <Alert className="mb-6">
              ...
              <AlertDescription>
                All purchases are subject to the platform&apos;s{' '}
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                return (
                  <Card key={listing.id} className="flex flex-col">
                    ...
                    <CardContent className="p-4 flex-grow flex flex-col">
                      ...
                      <p className="text-muted-foreground text-sm mt-1 flex-grow line-clamp-3">
                        {listing.description}
                      </p>
                    ...
                    <div className="p-4 pt-0 mt-auto flex items-center justify-between">
                      ...
                      <Button
                        ...
                        disabled={
                          !vendor.paymentsEnabled || isRedirectingToListing
                        }
                      >
                        {isRedirectingToListing && (
              ...
              {!areListingsLoading && listings?.length === 0 && (
                <p className="text-muted-foreground md:col-span-3">
                  This vendor has not added any listings yet.
                </p>
              )}
            </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <div className="md:col-span-1">
            <h2 className="text-[clamp(1.5rem,3.5vw,2.5rem)] font-bold font-headline leading-tight mb-2">
              Customer Reviews
            </h2>
            <p className="text-muted-foreground">
              See what others are saying about {vendor.businessName}.
            </p>
          </div>
                  ...
                  <CardHeader className="flex-row items-start gap-4 space-y-0">
                    <Avatar>
                      {avatar && (
                        <AvatarImage
                          src={avatar.imageUrl}
                          alt={review.residentName}
                          data-ai-hint={avatar.imageHint}
                        />
                      )}
                      <AvatarFallback>
                        {review.residentName.charAt(0)}
                      </AvatarFallback>
                    </Avatar>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-0.5">
                          {[...Array(5)].map((_, i) => (
                            <Star
                              key={i}
                              className={`h-4 w-4 ${i < review.rating ? 'text-amber-500 fill-amber-500' : 'text-muted-foreground/30'}`}
                            />
                          ))}
                        </div>
                      </div>
