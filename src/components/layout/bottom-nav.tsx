...
import { Home, LayoutGrid, ShoppingCart, Users, MoreHorizontal, Search, CalendarDays, MessageSquare, Filter } from "lucide-react";
  ...
  const getActions = () => {
    if (pathname.startsWith("/vendors")) {
      return [
        ...
        { title: "Filters", href: "/vendors#marketplace-filter-list", icon: Filter },
      ];
    }
    if (pathname.startsWith("/forums")) {
      return [
        { title: "Discussions", href: "/forums?tab=discussions", icon: MessageSquare },
        { title: "Events", href: "/forums?tab=events", icon: CalendarDays },
      ];
    }
    if (pathname.startsWith("/dashboard")) {
      return [
        { title: "Vendor Dashboard", href: "/dashboard/vendor", icon: LayoutGrid },
        { title: "Resident Dashboard", href: "/dashboard/resident", icon: LayoutGrid },
      ];
    }
    return [
      ...
      <ul className="flex items-center gap-2">
        ...
        <li className="flex">
          <Popover open={open} onOpenChange={setOpen}>
            ...
            <PopoverContent align="center" sideOffset={6} className="w-64 bg-background text-foreground">
