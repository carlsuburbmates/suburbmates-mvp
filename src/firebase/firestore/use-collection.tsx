 ...
 */
type FirestoreErrorType = 'permission' | 'network' | 'resource-exhausted' | 'unavailable' | 'unauthenticated' | 'unknown';
...
function createEnhancedError(
  ...
  try {
    ...
    console.warn('Failed to create enhanced error, falling back to original error:', creationError);
...
export function useCollection<T = any>(
  ...
  const hasValidQuery = useMemo(() => Boolean(memoizedTargetRefOrQuery), [memoizedTargetRefOrQuery]);
  ...
  const handleError = useCallback((error: FirestoreError) => {
    try {
      ...
      try {
        ...
        console.warn('Failed to create enhanced error, using original error:', enhancedCreationError);
      ...
      try {
        ...
        if (errorType === 'permission') {
          ...
          try {
            ...
            console.warn('Failed to create FirestorePermissionError for emitter:', permissionErrorCreationError);
  ...
  const retry = useCallback(() => {
    if (error && (error as any).isRetryable) {
      setIsRetrying(true);
      setIsLoading(true);
      setError(null);
    }
