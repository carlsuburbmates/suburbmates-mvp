...
const NaturalLanguageSearchInputSchema = z.object({
  query: z.string().describe('The user\'s natural language search query for finding a local business.'),
});
...
const NaturalLanguageSearchOutputSchema = z.object({
    category: z.string().optional().describe('The identified business category, if any. Must be one of: gardening, cafe, plumbing, retail, bakery, services.'),
    keywords: z.array(z.string()).optional().describe('A list of specific keywords or attributes extracted from the query (e.g., "wifi", "dog-friendly", "gluten-free").'),
...
const searchFilterTool = ai.defineTool(
  {
    ...
    description: 'Use this tool to provide the structured search filters based on the user\'s query.',
...
export async function naturalLanguageSearch(input: NaturalLanguageSearchInput): Promise<NaturalLanguageSearchOutput> {
  ...
  if (toolRequest && toolRequest.tool.name === 'setSearchFilters') {
