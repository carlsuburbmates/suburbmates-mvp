...
const ValidateAbnInputSchema = z.object({
  ...
  businessName: z
    .string()
    .describe('The business name to verify against the ABN.'),
})
...
const ValidateAbnOutputSchema = z.object({
  isValid: z
    .boolean()
    .describe('Whether the ABN is valid and the business name matches.'),
  message: z
    .string()
    .describe('A message indicating the result of the validation.'),
})
...
const abnLookupTool = ai.defineTool(
  {
    name: 'abnLookup',
    description:
      'Looks up an Australian Business Number (ABN) and returns the associated business name.',
  ...
  async (input) => {
    console.warn(`[ABN LIVE] Looking up ABN: ${input.abn}`)
    ...
    try {
      ...
      const data = JSON.parse(jsonText) as {
        ErrorMessage?: string
        BusinessName?: Array<{ IsCurrent?: boolean; Name?: string }>
      }
      ...
      const mainName = data.BusinessName?.find((n) => n.IsCurrent)
...
export async function validateAbn(
  input: ValidateAbnInput
): Promise<ValidateAbnOutput> {
