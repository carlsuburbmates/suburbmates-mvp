...
const SummarizeDisputeInputSchema = z.object({
  disputeReason: z
    .string()
    .describe(
      'The raw dispute reason provided by Stripe (e.g., "fraudulent", "product_not_received").'
    ),
  productName: z
    .string()
    .describe('The name of the product or listing being disputed.'),
  amount: z
    .number()
    .describe('The disputed amount in currency units (e.g., dollars).'),
})
const DisputeSummarySchema = z.object({
  summary: z
    .string()
    .describe(
      'A concise, one-sentence, human-readable summary of the dispute.'
    ),
  riskLevel: z
    .enum(['LOW', 'MEDIUM', 'HIGH'])
    .describe(
      'An assessment of the risk level to the platform or vendor.'
    ),
  recommendedAction: z
    .string()
    .describe(
      'A brief, actionable next step for the admin or vendor (e.g., "Advise vendor to submit shipping proof.").'
    ),
})
