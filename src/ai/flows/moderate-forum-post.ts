...
const ModerateForumPostInputSchema = z.object({
  postContent: z
    .string()
    .describe('The content of the forum post to moderate.'),
})
export type ModerateForumPostInput = z.infer<
  typeof ModerateForumPostInputSchema
>

const ModerateForumPostOutputSchema = z.object({
  isSafe: z
    .boolean()
    .describe('Whether the content is considered safe or not.'),
  reason: z
    .string()
    .optional()
    .describe('The reason why the content was blocked, if applicable.'),
})
export type ModerateForumPostOutput = z.infer<
  typeof ModerateForumPostOutputSchema
>
...
): Promise<ModerateForumPostOutput> {
...
const moderateForumPostFlow = ai.defineFlow(
  ...
  async (input) => {
    try {
      ...
      if (finishReason === 'SAFETY') {
        ...
        if (safetyRatings) {
          blockedCategories = safetyRatings
            ...
            .map(
              (rating) =>
                harmCategoryMap[rating.category] ||
                rating.category.replace('HARM_CATEGORY_', '').replace(/_/g, ' ')
            )
        }
      ...
      console.error('Moderation flow error:', err)
      throw err
    }
  }
)
